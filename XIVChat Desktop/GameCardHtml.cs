...
namespace XIVChat_Desktop {
    internal sealed class ItemCardState {
        ...
        internal string Name => this.ShowChinese && !string.IsNullOrEmpty(this.Chinese?.Name) ? this.Chinese.Name : this.Item.ItemName ?? this.Item.Content;
        internal string Text(string table, uint id, string original) => this.ShowChinese && this.Names.TryGetValue(table, out var names) && names.TryGetValue(id, out var value) ? value : original;
        internal string Parameter(uint id, string original) => this.ShowChinese && this.Chinese?.Parameters.TryGetValue(id, out var value) == true ? value : this.Text("BaseParam", id, original);
    ...
    internal static class GameCardHtml {
        ...
        internal static string Copy(ItemCardState state) {
            ...
            return state.Name + (hq ? " HQ" : "") + $" (#{item.ItemId})\n" +
                string.Join("\n", item.ItemDetails?.Parameters.Select(p => state.Parameter(p.Id, p.Name) + " " + p.Value(hq)) ?? item.ItemStats ?? new List<string>());
        }
        internal static string Build(ItemCardState state) {
            ...
            string category = state.ShowChinese && !string.IsNullOrEmpty(state.Chinese?.Category) ? state.Chinese.Category : item.ItemCategory ?? "";
            string jobs = state.ShowChinese && !string.IsNullOrEmpty(state.Chinese?.ClassJobs) ? state.Chinese.ClassJobs : details?.ClassJobs ?? "";
            string description = state.ShowChinese && !string.IsNullOrEmpty(state.Chinese?.Description) ? state.Chinese.Description : item.ItemDescription ?? "";
            ...
            var stats = details?.Parameters.Where(p => p.Value(hq) != 0).Select(p => state.Parameter(p.Id, p.Name) + $" {p.Value(hq):+0;-0;0}").ToArray() ?? item.ItemStats?.ToArray() ?? Array.Empty<string>();
        ...
        private static void AppendRecipes(StringBuilder body, ItemCardState state) {
            ...
            if (state.RecipesLoading) body.Append($"<div class='muted'>{E(L("Card.Loading"))}</div>");
        ...
        private static void AppendSources(StringBuilder body, ItemCardState state) {
            ...
            if (state.SourcesLoading) body.Append($"<p class='muted'>{E(L("Card.Loading"))}</p>");
